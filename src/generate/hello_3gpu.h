...
graph_t hello_3gpu(uint64_t ni)
{
  ...
  ident_t y1 = g.insert(apply_t {.loc = gpu1, .read_mems = {mem0, mem1}, .write_mems = {mem2}, .op = gen_gpu_matmul(ni,ni,ni) }, {move_x_1, move_y_1});
  ident_t move_y_2 = g.insert(sendrecv_t { .src=gpu1, .dst=gpu2, .src_mem=mem2, .dst_mem=mem1}, {y1});

  ident_t y2 = g.insert(apply_t {.loc = gpu2, .read_mems = {mem0, mem1}, .write_mems = {mem2}, .op = gen_gpu_matmul(ni,ni,ni) }, {move_x_2, move_y_2});
  ident_t move_y_3 = g.insert(sendrecv_t { .src=gpu2, .dst=gpu3, .src_mem=mem2, .dst_mem=mem1}, {y2});

  ident_t y3 = g.insert(apply_t {.loc = gpu3, .read_mems = {mem0, mem1}, .write_mems = {mem2}, .op = gen_gpu_matmul(ni,ni,ni) }, {move_x_3, move_y_3});

  ident_t move_y_cpu_1 = g.insert(sendrecv_t { .src=gpu1, .dst=cpu, .src_mem=mem2, .dst_mem=mem0}, {y1});
  ident_t move_y_cpu_2 = g.insert(sendrecv_t { .src=gpu2, .dst=cpu, .src_mem=mem2, .dst_mem=mem1}, {y2});
  ident_t move_y_cpu_3 = g.insert(sendrecv_t { .src=gpu3, .dst=cpu, .src_mem=mem2, .dst_mem=mem2}, {y3});

  g.insert(apply_t{.loc = cpu, .read_mems = {mem0}, .write_mems = {}, .op = gen_print({ni,ni}) }, {move_y_cpu_1});
  g.insert(apply_t{.loc = cpu, .read_mems = {mem1}, .write_mems = {}, .op = gen_print({ni,ni}) }, {move_y_cpu_2});
  g.insert(apply_t{.loc = cpu, .read_mems = {mem2}, .write_mems = {}, .op = gen_print({ni,ni}) }, {move_y_cpu_3});
